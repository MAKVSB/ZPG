#include <vector>

std::vector<float> triangleVertexData = {
	//// back 
	-.5f, -.5f, .5f, 1, 1.0f, .0f, .0f, 1,
	.0f, .5f, .0f, 1, 1.0f, .0f, .0f, 1,
	.5f, -.5f, .5f, 1, 1.0f, .0f, .0f, 1,
	//// left
	-.5f, -.5f, .5f, 1, .0f, 1.0f, .0f, 1,
	-.5f, -.5f, -.5f, 1, .0f, 1.0f, .0f, 1,
	.0f, .5f, .0f, 1, .0f, 1.0f, .0f, 1,
	//// right
	.5f, -.5f, .5f, 1, .0f, .0f, 1.0f, 1,
	.5f, -.5f, -.5f, 1, .0f, .0f, 1.0f, 1,
	.0f, .5f, .0f, 1, .0f, .0f, 1.0f, 1,
	// bottom
	.5f, -.5f, .5f, 1, .0f, 1.0f, 1.0f, 1,
	-.5f, -.5f, .5f, 1, .0f, 1.0f, 1.0f, 1,
	-.5f, -.5f, -.5f, 1, .0f, 1.0f, 1.0f, 1,
	.5f, -.5f, .5f, 1, .0f, .5f, .5f, 1,
	.5f, -.5f, -.5f, 1, .0f, .5f, .5f, 1,
	-.5f, -.5f, -.5f, 1, .0f, .5f, .5f, 1,
	//// front 
	-.5f, -.5f, -.5f, 1, 1.0f, .0f, 1.0f, 1,
	.0f, .5f, -.0f, 1, 1.0f, .0f, 1.0f, 1,
	.5f, -.5f, -.5f, 1, 1.0f, .0f, 1.0f, 1,

	////// back 
	//-.5f, 1-.5f, .5f, 1, 1.0f, .0f, .0f, 1,
	//.0f, 1.5f, .0f, 1, 1.0f, .0f, .0f, 1,
	//.5f, 1-.5f, .5f, 1, 1.0f, .0f, .0f, 1,
	////// left
	//-.5f, 1-.5f, .5f, 1, .0f, 1.0f, .0f, 1,
	//-.5f, 1-.5f, -.5f, 1, .0f, 1.0f, .0f, 1,
	//.0f, 1.5f, .0f, 1, .0f, 1.0f, .0f, 1,
	////// right
	//.5f, 1-.5f, .5f, 1, .0f, .0f, 1.0f, 1,
	//.5f, 1-.5f, -.5f, 1, .0f, .0f, 1.0f, 1,
	//.0f, 1.5f, .0f, 1, .0f, .0f, 1.0f, 1,
	//// bottom
	//.5f, 1-.5f, .5f, 1, .0f, 1.0f, 1.0f, 1,
	//-.5f, 1-.5f, .5f, 1, .0f, 1.0f, 1.0f, 1,
	//-.5f, 1-.5f, -.5f, 1, .0f, 1.0f, 1.0f, 1,
	//.5f, 1-.5f, .5f, 1, .0f, .5f, .5f, 1,
	//.5f, 1-.5f, -.5f, 1, .0f, .5f, .5f, 1,
	//-.5f, 1-.5f, -.5f, 1, .0f, .5f, .5f, 1,
	////// front 
	//-.5f, 1-.5f, -.5f, 1, 1.0f, .0f, 1.0f, 1,
	//.0f, 1.5f, -.0f, 1, 1.0f, .0f, 1.0f, 1,
	//.5f, 1-.5f, -.5f, 1, 1.0f, .0f, 1.0f, 1,
};